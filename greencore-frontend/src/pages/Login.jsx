...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const result = await login({
      email: sanitizeInput(data.email), // Solo email viene sanitized
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-themed-muted mt-1">
          Accedi al tuo account AUREA
        </p>
      </div>

      {/* ── OAuth Buttons (identical styling) ── */}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex justify-end mt-1.5">
            <Link
              to="/forgot-password"
              className="text-xs text-aurea-500 hover:text-aurea-400 transition-colors"
            >
